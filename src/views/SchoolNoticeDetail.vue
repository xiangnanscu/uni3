...
export default {
  ...
  methods: {
    ...
    async onDeleteConfirm() {
      const { affected_rows } = await usePost(`/school_notice/delete/${this.query.id}`);
      if (affected_rows === 1) {
        this.$refs.deleteConfirmRef.close();
        uni.showToast({ title: "操作成功" });
        utils.redirect("SchoolNotice", 1000);
      } else {
        uni.showToast({ title: "删除失败" });
      }
    },
    async fetchData(query) {
      const record = await useGet(`/school_notice/detail/${query.id}`);
      this.record = record;
    },
    async deleteRecordPop() {
      await usePost(`/school_notice/delete/${this.query.id}`);
    },
  },
};
</script>
