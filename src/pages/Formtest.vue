...
export default {
  ...
  computed: {
    ...
    filteredChoices() {
      return this.baseFormData.name
        ? this.namechoices.filter((e) =>
            e.value.includes(this.baseFormData.name)
          )
        : [];
    }
  },
  async onLoad() {
    const { data } = await Http.get(
      "https://zzb.jazzb.cn/system/dept/outlist",
      { foo: "bar" }
    );
