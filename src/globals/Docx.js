...
async function toBuffer({ filename, buffer, url, data, context, ...options }) {
  const template =
    buffer ?? docxTemplates[filename] ?? (await getDocxTemplate(url));
  if (!template) {
    throw new Error(
      `you at least provide filename or buffer or url to toBuffer`
    );
  }
...
async function saveTemplate({
  ...
  saveBufferToFile(
    docxBuffer,
    !filename.endsWith(".docx") ? filename + ".docx" : filename
  );
  return docxBuffer;
}
...
async function uploadBufferToOss({ buffer, filename, prefix }) {
  ...
  const { data: payload } = await Axios.post(process.env.ALIOSS_PAYLOAD_URL, {
    key,
    size: "10m",
  ...
  await Axios.post(process.env.ALIOSS_URL, body);
